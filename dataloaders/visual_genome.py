...
def load_graphs(graphs_file, mode='train', num_im=-1, num_val_im=0, filter_empty_rels=True,
    ...
    counts = vg_dict_info['predicate_count']
    predicates_sort = sorted(counts.items(), key=lambda x:x[1], reverse=True)
    ...
    for i in range(len(image_index)):
        ...
        if root_classes is not None and mode == 'train':
            ...
            # print('old rels: ', rels)
            n_threshold = 1000
            rel_temp = predicate_debiasing(mode, rels, ind_to_predicates, root_classes_count, leaf_classes_count, all_classes_count, n_threshold, root_classes)
            if len(rel_temp) == 0:
                split_mask[image_index[i]] = 0
                continue
            else:
...
def predicate_debiasing(mode, rels, ind_to_predicates, root_classes_count, leaf_classes_count, all_classes_count, n_threshold=1000, root_classes=None):
    rel_temp = []
